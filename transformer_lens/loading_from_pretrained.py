...
def convert_hf_model_config(model_name: str, **kwargs):
    ...
    if "llama" in official_model_name.lower() and "tinyllama" not in official_model_name.lower():
        architecture = "LlamaForCausalLM"
    elif "gemma" in official_model_name.lower():
        architecture = "GemmaForCausalLM"
    else:
        huggingface_token = os.environ.get("HF_TOKEN", None)
        if "local_path" in kwargs.keys() and kwargs["local_path"] is not None:
            hf_config = AutoConfig.from_pretrained(kwargs["local_path"], **kwargs)
        else:
    ...
    **kwargs,
    ...
    if (
        ...
        if cfg.original_architecture == "GPT2LMHeadModel":
            ...
            if 'tinyllama' in official_model_name:
                state_dict = convert_tinyllama_weights(hf_model, cfg)
            else:
...
def convert_tinyllama_weights(llama, cfg: HookedTransformerConfig):
    state_dict = {}
